"""Claude Code CLI backend for review generation — no API key required.

Shells out to the local ``claude`` binary (Claude Code, headless mode) so
Frank can draft reviews, answer RLM ``complete()`` calls, and run security
triage assessments without an Anthropic API key. Billing happens through the
CLI's own authenticated session (subscription or whatever credentials the
operator already logged in with), so every call this backend makes is
...
standard :class:`LLMBackend` interface so the Claude Code CLI can be
selected as an ordinary ``provider: "claude-code"`` backend everywhere
backends are already pluggable: the review drafter, the RLM broker, and
``metered_call`` sites like tone guard and security triage.
"""
...
class ClaudeCodeBackend(BaseLLMBackend):
    """Review backend that shells out to the local Claude Code CLI.
    ...
    the ``claude`` binary the operator already has installed and logged
    into (Claude Code headless mode: ``claude -p ... --output-format
    json``).
    """
    ...
    def __init__(self, config: LLMBackendConfig) -> None:
        ...
        # `_sdk_available` from whether the CLI binary is actually on PATH,
        # so the base class's graceful-degradation paths (empty findings,
        # "" completion) kick in the same way they do for a missing SDK
        # package.
        self._sdk_available = shutil.which(self._cli_path) is not None
        if not self._sdk_available:
            logger.error(
                "claude CLI binary %r not found on PATH; claude-code backend disabled.",
                self._cli_path,
            )
